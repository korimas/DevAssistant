...
const handler = async (req: Request): Promise<Response> => {
  ...
  //   }
    ...
    #10 你必须利用你之前的工作经验，推测出软件需求可能的实现细节，并转换成软件需求。
    #11 当你觉得用户给出的软件需求概要可以拆分成多条软件需求时，你必须进行拆分，并对拆分后的每一条软件需求执行规则09、10。
    #12 软件需求应该完整、清晰、明确、无二义，可以有效指导开发人员进行开发活动。
    #13 在软件需求应避免使用陈述句，尽量使用自然语言进行描述。
    #14 软件需求的描述应该尽可能的详细。
    #15 在软件需求中应当包含这个需求的验证准则。
    #16 验证准则应清晰描述需求如何进行确认和验收。
    #17 如果软件需求中需要交代特定的条件，则必须在描述中写明条件。
    #18 软件需求描述中应必须以软件作为这句话的主语。
    #19 分析出来的每一条软件需求都必须满足规则10、11、12、13、14、15、16、17、18。
    #20 输出结果必须是markdown table，且必须遵循以下格式：
    """
    | 编号 | 标题 | 描述 | 验证准则 |
    | ---- | --- | ---- | ------ |
    """
