...
export async function RequestStream(payload: GPTAPIRequest) {
  ...
  console.log('RequestStream payload:', payload);
  ...
  if (!res.ok) {
    ...
    console.error('Failed to fetch from DeepSeek:', res.status, res.statusText, content);
    throw new Error(`Failed to fetch from DeepSeek: ${res.status} ${res.statusText}`);
  }
